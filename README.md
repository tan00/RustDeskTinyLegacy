...
本仓库是基于 RustDesk **1.3.7 冻结基线**（源码提交 `1f02bc9d`）的独立定制分支，面向 **Windows 7 SP1 x64**，**只支持 IP 地址直连** 的远程桌面能力，不跟随上游后续版本。与原版的主要差异：

- **仅支持 IP:端口直连**：只接受数字 IPv4 或带方括号的 IPv6 地址加非零端口；**屏蔽了一切与 RustDesk 官方服务器或自建服务器的通信**——不注册会合/中继服务器、不使用 RustDesk ID、无 NAT/延迟探测、账号同步和软件更新请求，空闲时无任何对外探测流量，不会向任何 RustDesk 服务器发送信息。两台设备之间的连接数据也不经过任何第三方服务器。
- **直连监听端口改为 21121**（`RENDEZVOUS_PORT = 21119`，直连端口 = `RENDEZVOUS_PORT + 2`），避免与原版 RustDesk 占用的 21116–21119 冲突，两者可共存。可在设置中自定义直连端口。
