...
| `src/rendezvous_mediator.rs` | Replaces rendezvous/relay registration with a direct TCP listener that feeds the shared 1.3.7 server implementation. |
...
| `flutter/lib/desktop/pages/desktop_home_page.dart` | Hides the local ID, preserves the one-time password and shows a password-specific explanation. |
