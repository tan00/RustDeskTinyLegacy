...
pub fn parse_host_args(args: &[String]) -> Result<Option<SocketAddr>, String> {
    ...
    if args.len() != 3 || args[1] != "--listen" {
        return Err("usage: RustDeskTinyLegacy host --listen <ip:port>".to_owned());
    }
...
pub fn listen_address() -> Result<SocketAddr, String> {
    let value = std::env::var(LISTEN_ENV)
        ...
        .map_err(|_| "RustDeskTinyLegacy host listener requires an explicit ip:port".to_owned())?;
    parse_address(&value)
}
...
fn service_listen_path() -> std::path::PathBuf {
    #[cfg(target_os = "linux")]
    return "/var/lib/rustdesktinylegacy/direct-listen".into();
    #[cfg(target_os = "macos")]
    return "/Library/Application Support/RustDeskTinyLegacy/direct-listen".into();
}
...
pub async fn configure_service_host(address: SocketAddr) -> Result<(), String> {
    let mut stream = crate::ipc::connect(3000, crate::POSTFIX_SERVICE)
        .await
    ...
    Err(format!(
        "RustDeskTinyLegacy service did not listen on {address} within 15 seconds"
    ))
}
